...
function tinhTienThue() {
  ...
  function formatCurrency(tienThuNhapCaNhan) {
    new Intl.NumberFormat('vn-VN', { style: 'currency', currency: 'JPY' }).format(
      tienThuNhapCaNhan,
    )
  }
  if (thuNhapNam > 0 && thuNhapNam <= 60e6) {
    tienThuNhapCaNhan =
      (thuNhapNam - thuNhapNam * 0.05 - 4e5 - soNguoiPhuThuoc * 1.6e5) / 12;

    return (thuNhapChiuThue.innerHTML = `Họ tên: ${hoTen}, Tiền thuế thu nhập cá nhân: ${new Intl.NumberFormat(
      "ja-JP",
      { style: "currency", currency: "JPY" }
    ).format(tienThuNhapCaNhan)} VND `);
  }
  if (thuNhapNam > 60e6 && thuNhapNam <= 120e6) {
    tienThuNhapCaNhan =
      (thuNhapNam -
        (60e6 * 0.05 + (thuNhapNam - 60) * 0.1) -
        4e5 -
  ...
  if (thuNhapNam > 120e6 && thuNhapNam <= 210e6) {
    tienThuNhapCaNhan =
      (thuNhapNam -
        (60e6 * 0.05 + 60e6 * 0.1 + (thuNhapNam - 120) * 0.15) -
        4e5 -
        soNguoiPhuThuoc * 1.6e5) /
      12;
    return (thuNhapChiuThue.innerHTML = `Họ tên: ${hoTen}, Tiền thuế thu nhập cá nhân: ${tienThuNhapCaNhan} VND `);
  ...
  if (thuNhapNam > 210e6 && thuNhapNam <= 384e6) {
    tienThuNhapCaNhan =
      (thuNhapNam -
        (60e6 * 0.05 + 60e6 * 0.1 + 90e6 * 0.15 + (thuNhapNam - 210) * 0.2) -
        4e5 -
        soNguoiPhuThuoc * 1.6e5) /
      12;
    return (thuNhapChiuThue.innerHTML = `Họ tên: ${hoTen}, Tiền thuế thu nhập cá nhân: ${tienThuNhapCaNhan} VND `);
